...
Supports macOS (64-bit and arm64), Linux (32 and 64-bit, armhf, arm64) and Windows (32 and 64-bit). [The ffmpeg version currently used is `8.0.1`.](https://github.com/alienartio/ffmpeg-ffprobe-static/releases/tag/b8.0.1)
...
*Note:* During installation, it will download the appropriate `ffmpeg` binary from the [`b8.0.1` GitHub release](https://github.com/alienartio/ffmpeg-ffprobe-static/releases/tag/b8.0.1). Use and distribution of the binary releases of FFmpeg are covered by their respective license.

Alternatively, it will fetch binaries from `FFMPEG_FFPROBE_STATIC_BASE_URL` if set as an environment variable. The default base URL is https://github.com/alienartio/ffmpeg-ffprobe-static/releases/download/. The install script will fetch binaries from `BASE_URL/binary-release-tag` (the `binary-release-tag` is set in package.json and can customized by setting `FFMPEG_BINARY_RELEASE`).
...
- [Windows x86 builds](https://github.com/defisym/FFmpeg-Builds-Win32/)
- [Linux builds](https://johnvansickle.com/ffmpeg/)
- macOS builds [for Intel](https://osxexperts.net/) / [for ARM (Apple Silicon)](https://osxexperts.net/)
...
- **macOS builds**: [osxexperts.net](https://osxexperts.net/)
